...
def generate_text_report(results) -> str:
    ...
    if results.sensitivity is not None:
        ...
        for param, (low, high) in sorted(
            results.sensitivity.tornado_data.items(),
...
def generate_plots(results, output_dir: str = "."):
    ...
    has_drr = results.drr is not None
    ncols = 3
    ...
    # 5. Tornado Chart
    ax = axes[1, 1]
    if results.sensitivity is not None and results.sensitivity.tornado_data:
